  ...
  return async function (ctx, next) {
    return next().then(async () => {
      try {
        ...
        const arr = ctx.url.substr(0, ctx.url.indexOf('?')).split('/').filter(v => !!v);
        ...
        if (share.config.requestLog) {
          ...
          LOGGER.warn(`response, exec: ${time(start)}, size: ${size(JSON.stringify(result).length)}, data: ${JSON.stringify(result)}`);
        }
