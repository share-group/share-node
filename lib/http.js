...
module.exports = class {
  ...
  constructor() {
    ...
    global.share.config = require(configFile);
    ...
    share.config.logger = share.config.logger || defaultConfig.logger;
  ...
  start(...middleware) {
    ...
    // load app's middlewares
    middleware.map(m => app.use(m));

    // load share's middlewares
    app.use(require('./middleware/requestStatsd')());
    app.use(require('./middleware/requestMonitor')());
  ...
  [print]() {
    ...
    const _version = `share node version: ${pkg.version}, node version: ${process.version}, env: ${share.ENV}, log level: ${share.config.logger.level}`;
